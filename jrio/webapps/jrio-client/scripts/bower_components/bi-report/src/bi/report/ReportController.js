...
define(function (require) {
    ...
    var Backbone = require("backbone"),
        ...
        PageComponentMetaCollection = require("./jive/collection/PageComponentMetaCollection"),
        ...
        this.model
            ...
            .then(function() {
                if (self.model.isFailed() || self.model.isCancelled()) {
                    ...
                    self.fetchPageHtmlExportAndJiveComponents({silent: action.silent})
                        .done(dfd.resolve)
                        .fail(dfd.reject);
                }
            }, dfd.reject);
    ...
    function ReportController(stateModel) {
        ...
        this.listenTo(this.model, "change:pageStatus", function() {
            ...
            if (!self.model.isReady() && !htmlExport.get("outputFinal")
    ...
    _.extend(ReportController.prototype, Backbone.Events, {
        ...
        executeReport: function(refresh) {
            ...
            this.model
                ...
                .then(_.bind(self.fetchPageHtmlExportAndJiveComponents, self), dfd.reject)
                .then(function() {
                    self.stateStack.newState();
        ...
        applyReportParameters: function(refresh) {
            ...
            this.model
                ...
                .then(function() {
                    ...
                    self.fetchExportDfd = self.fetchPageHtmlExportAndJiveComponents({silent: true});
                ...
                exportDfd.then(function () {
                    if (htmlExport.isFailed() || htmlExport.isCancelled()) {
                        ...
                        outputDfd
                            ...
                            .then(_.bind(self.pageComponentsMeta.fetch, self.pageComponentsMeta), dfd.reject)
